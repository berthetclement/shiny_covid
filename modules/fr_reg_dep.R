  ...
  moduleServer(
    ...
    function(input, output, session){
      ...
      global_france <- reactive({
        ...
        for(i in seq(length(df_g_fr$date))){
          if(i>=4){
            # deces
            if( df_g_fr$deces_tot[i] < mean(df_g_fr$deces_tot[i:(i-4)], na.rm=TRUE) ){
              df_g_fr$deces_tot[i] = NA
            }
          }
        }
        df_g_fr
      ...
      gt_reg_france <- reactive({
        ...
        df_gt_reg_fr$region[df_gt_reg_fr$region %in% "La_Reunion"] <- "La R\u00e9union"
      ...
      observeEvent(input$niv_select, {
          ...
          if(input$niv_select %in% "R\u00e9gions"){
            ...
            # observeEvent(input$id_slider_date, {})
          ...
          if(input$niv_select %in% "Global"){
            ...
            output$glob_hospi <- renderPlotly({
              ...
              num_var <- sym("hospitalises")
              cumul_plotly(global_france(), num_var, "Hospitalis\u00e9s", colgraph)
            ...
            output$glob_new_hospi <- renderPlotly({
              df_g_n_h <- data_full_fr %>% 
                filter(granularite %in% "pays") %>% 
                filter(source_type %in% "opencovid19-fr") %>% 
                filter(date >= as.Date(input$id_slider_date) & date <= as.Date(date_in[2]))
              
              if( (is.na(df_g_n_h$nouvelles_hospitalisations) & length(df_g_n_h$nouvelles_hospitalisations) == 1))
                stop(paste0("Pas de donn\u00e9es disponible pour le ", df_g_n_h$date))
              
              num_var <- sym("nouvelles_hospitalisations")
              cumul_new_plotly(df_g_n_h, num_var, "Nouveaux hospitalis\u00e9s", colgraph)
            ...
            output$glob_rea <- renderPlotly({
              ...
              num_var <- sym("reanimation")
              cumul_plotly(global_france(), num_var, "R\u00e9animations", colgraph)
            ...
            output$glob_new_rea <- renderPlotly({
              df_g_n_r <- data_full_fr %>% 
                filter(granularite %in% "pays") %>% 
                filter(source_type %in% "opencovid19-fr") %>% 
                filter(date >= as.Date(input$id_slider_date) & date <= as.Date(date_in[2]))
              
              if( (is.na(df_g_n_r$nouvelles_reanimations) & length(df_g_n_r$nouvelles_reanimations) == 1))
                stop(paste0("Pas de donn\u00e9es disponible pour le ", df_g_n_r$date))
              
              num_var <- sym("nouvelles_reanimations")
              cumul_new_plotly(df_g_n_r, num_var, "Nouvelles r\u00e9animations", colgraph)
          ...
          if(input$niv_select %in% "D\u00e9partements"){
            ...
            gt_dep_france <- reactive({
              ...
              # remise accent pour nom REUNION
              df_dep$region[df_dep$region %in% "La_Reunion"] <- "La R\u00e9union"
